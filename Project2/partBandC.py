...
def one_hidden_layer_sigmoid(X_train, X_test, z_train, z_test):
    ...
    In this test we study:
        ...
        8. Searching Eta=0.9 Vs Decays;
        9. Searching Eta=0.9 and Decay=0 Vs Lambdas;
    """
...
def two_hidden_layer_sigmoid(X_train, X_test, z_train, z_test):
    ...
    """
    n_neurons = [50, 100, 150, 200, 300, 400, 500]
...
def two_hidden_layer_tanh(X_train, X_test, z_train, z_test):
    ...
    for d in decays:
        md = MLP(hidden_layers=[500, 500], epochs=500,
                 batch_size=len(X_train), eta0=0.05,
                 learning_rate='decay', decay=d, lmbd=0.0,
                 bias0=0.01, init_weights='normal',
    ...
    plt.ylabel("MSE scores")
    plt.xlabel("Decay values")
    plt.title("Training and Testing MSE Vs Decays for $\eta$=0.05")
    plt.legend()
    plt.grid()
    plt.ylim([8, 16])
    plt.show()

    # tuning Eta=0.05 and Decay=1e-6 Vs Lambdas;
    lmbds = [0, 10 ** -7, 10 ** -6, 10 ** -5, 10 ** -4, 10 ** -3]
    ...
    plt.ylabel("MSE scores")
    plt.xlabel("Lambdas values")
    plt.title("Training & Testing MSE Vs Lambdas for $\eta$=0.05 "
              "and decay=1e-6")
    plt.legend()
    plt.grid()
    plt.show()
...
def two_hidden_layer_relu(X_train, X_test, z_train, z_test):
    ...
    In this test we study:
        ...
        7. Tuning Eta=0.05 and Decay=0 Vs Lambdas;
    """
    n_neurons = [7, 8, 9, 10, 14, 20]
    etas = [0.001, 0.0009, 0.0008, 0.0007, 0.0006, 0.0005, 0.0001]
    ...
    plt.plot(decays[:-2], mse_tr[:-2], label='MSE Train')
    plt.plot(decays[:-2], mse_te[:-2], "--", label='MSE Test')
    plt.ylabel("MSE scores")
    plt.xlabel("Decay values")
    plt.title("Training and Testing MSE Vs Decays for $\eta$=0.08")
    plt.legend()
    ...
    # tuning Eta=0.0005 and Decay=1e-6 Vs Lambdas;
    lmbds = [0, 10 ** -7, 10 ** -6, 10 ** -5, 10 ** -4, 10 ** -3]
...
if __name__ == '__main__':
    ...
    # 2st study - 2 hidden-layers - sigmoid
    two_hidden_layer_sigmoid(X_train, X_test, z_train, z_test)

    # 3st study - 1 hidden-layers - tanh

    # 4st study - 2 hidden-layers - tanh
    two_hidden_layer_tanh(X_train, X_test, z_train, z_test)

    # 5st study - 1 hidden-layers - relu

    # 6st study - 2 hidden-layers - relu
